...
public interface TaskMapper extends BaseMapper<Task> {
    ...
    @Select(" select count(a.id) id from pear_task a where a.project_code = #{projectCode} and (a.create_time BETWEEN #{beginTime} and #{endTime}) ")
    Integer selectDateTaskTotalForProject(String projectCode,String beginTime,String endTime);

    @Select("SELECT  id,code,project_code,name,pri,execute_status,description,create_by,create_time,assign_to,deleted,stage_code,task_tag,done,begin_time,end_time,remind_time,pcode,sort,`liked`,star,deleted_time,private,id_num,path,schedule,version_code,features_code,work_time,status  FROM pear_task     WHERE project_code = #{projectCode} AND deleted = #{deleted}")
