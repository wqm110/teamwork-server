            ...
            public void handle(int sheetIndex, int rowIndex, List<Object> rowlist) {
                if(rowIndex>=dataStartRow-1){
                    ...
                    if(ObjectUtils.isEmpty(member)){
                        ...
                        memberAccount = memberAccountService.inviteMember(
                                MemberAccount.builder()
                                        .member_code(member.getCode())
                                        .organization_code(orgCode)
                                        .position(ListUtils.getValue(rowlist,3,String.class))
                                        .mobile(ListUtils.getValue(rowlist,4,String.class))
                                        .description(ListUtils.getValue(rowlist,6,String.class))
                                        .build());
                    }else{
                        ...
                        if(null != deps){
                            for(String dep:deps){
                                ...
                                if(null != deptNames){
                                    ...
                                    if(!ObjectUtils.isEmpty(departMent)){
                                        inviteMember(memberAccount.getCode(), departMent.getCode(),0, 0, orgCode);
                                    }
                                }
                            }
                        }else{
                            inviteMember(memberAccount.getCode(), "",0, 0, orgCode);
                        }
                    }
                }
            }
